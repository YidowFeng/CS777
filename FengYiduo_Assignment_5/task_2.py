...
if __name__ == "__main__":
    ...
    file_GD = file.map(lambda x: (float(x[5]), float(x[11])))
    ...
    for i in range(numIteration):
        file_GD = file_GD.map(lambda x: (x[0], (x[1] - (m*x[0]+b))))
        ...
        re_2.append([i, " m=", m, " b=", b, " cost=", cost])
    task_2 = sc.parallelize(re_2).coalesce(1).saveAsTextFile(sys.argv[2])
