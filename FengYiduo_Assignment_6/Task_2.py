...
# ----------------------------------------------------------------
	...
	for i in range(max_iteration):
		...
		pred = pred.treeAggregate((np.zeros(coefficient_size), 0, 0),
				lambda x, y: (x[0] + y[1] * (-y[0] + (1 - 1 / (np.exp(np.dot(y[1], cur_coef)) + 1))),
				x[1] + y[0] * (-(np.dot(y[1], cur_coef))) + np.log(1 + np.exp(np.dot(y[1], cur_coef))),
						 x[2] + 1), add)
	...
	dictionary_tmp = dict((x, y) for y, x in dictionary_tmp.items())
	for p in coefficient.argsort()[-5:]:
		print(dictionary_tmp.get(p), "   coefficient:", coefficient[p])
